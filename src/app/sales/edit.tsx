...
export default function EditSaleScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const itemsForm: SaleItemForm[] = saleData.items.map(item => ({
        ...
        promo_price: item.promo_price ? item.promo_price.toFixed(2) : undefined,
        subtotal: item.subtotal.toFixed(2),
        qty_with_discount: '0',
        qty_without_discount: item.quantity.toString()
      }));
  ...
  const handleSubmit = async () => {
    ...
    if (validItems.length === 0 || !formData.customer_name.trim() || !formData.payment_method) {
    ...
    try {
      setSaving(true);
      await SaleService.update(user!.id, id, {
      ...
      const updatedSale = await SaleService.getById(user!.id, id);
      if (updatedSale) {
